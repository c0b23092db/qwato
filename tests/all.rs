...
#[test]
fn normal_all() {
    let output = run_all(&["--all", "--limit", "30"]);
    let lines = output_lines(&output);

    assert_eq!(lines.first(), Some(&"2025-07-29"));
    assert_eq!(lines.last(), Some(&"\t#tag1 #tag2 25:25:30 0725.no"));
    assert!(lines.windows(3).any(|pair| pair
        == [
...
#[test]
fn all_tag1() {
    let output = run_all(&["--all", "--tag", "tag1", "--limit", "10"]);
    let lines = output_lines(&output);
    assert_eq!(
        lines,
        vec![
...
#[test]
fn all_tag2() {
    let output = run_all(&["--all", "--tag", "tag2", "--limit", "15"]);
    let lines = output_lines(&output);
    assert_eq!(
        lines,
        vec![
...
#[test]
fn all_tag1_and_tag2() {
    let output = run_all(&["--all", "--tag", "tag1,tag2", "--limit", "15"]);
    let lines = output_lines(&output);
    assert_eq!(
        lines,
        vec![
...
#[test]
fn all_tag3_has_no_results() {
    let output = run_all(&["--all", "--tag", "tag3", "--limit", "10"]);
    let lines = output_lines(&output);
